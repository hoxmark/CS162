...
case class UserType(un: UserDefinedTypeName, taus: List[Type]) extends Type
case class TypeVariableType(t: TypeVariable) extends Type
...
case class NamedCallExp(fn: FunctionName, taus: List[Type], e: Exp) extends Exp
...
case class ConstructorExp(cn: ConstructorName, taus: List[Type], e: Exp) extends Exp
...
case class UserDefinedTypeDef(un: UserDefinedTypeName, ts: List[TypeVariable], cdefs: Seq[ConstructorDefinition])
...
case class Def(fn: FunctionName, ts: List[TypeVariable], x: Variable, tau1: Type, tau2: Type, e: Exp)
